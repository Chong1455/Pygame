...
def main():
    global FPSCLOCK, DISPLAYSURF, REDPILERECT, BLACKPILERECT, REDTOKENIMG, BLACKTOKENIMG, BOARDIMG, \
        ARROWIMG, ARROWRECT, HUMANWINNERIMG, COMPUTERWINNERIMG, WINNERRECT, TIEWINNERIMG
...
def runGame(isFirstGame):
    if isFirstGame:
        #Let the computer go first on the first game, so the player can see how the tokens
        #are dragged from the token piles
        turn = COMPUTER
        showHelp = True
    else:
...
def getHumanMove(board, isFirstMove):
    ...
    while True:
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if tokeny < YMARGIN and tokenx > XMARGIN and tokenx < WINDOWWIDTH - XMARGIN:
                    #left go at the top of the screen
        ...
        if tokenx != None and tokeny != None:
            drawBoard(board, {'x': tokenx - int(SPACESIZE /2),
...
def isValidMove(board, column):
    #Returns True if there is an empty space in the give column
