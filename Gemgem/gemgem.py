...
def main():
    ...
    for i in range(1, NUMGEMIMAGES+1):
        ...
        if gemImage.get_size() != (GEMIMAGESIZE, GEMIMAGESIZE):
            gemImage = pygame.transfrom.smoothscale(gemImage, (GEMIMAGESIZE, GEMIMAGESIZE))
    ...
    #Create pygame.Rect objects for each board space to do
    #board-coordinate-to-pixel-coordinate conversions
    BOARDRECTS = []
    for x in range(BOARDWIDTH):
        BOARDRECTS.append([])
        for y in range(BOARDHEIGHT):
            r = pygame.Rect((XMARGIN + (x * GEMIMAGESIZE), YMARGIN + (y * GEMIMAGESIZE),
...
def runGame():
    #Plays thorugh a single game. When the game is over, this function returns
    ...
    while True: #main game loop
        ...
        for event in pygame.event.get():
            if event.type == QUIT or (event.type == KEYUP and event.key == K_ESCAPE):
                ...
                if gameIsOver:
                    return #after game ends, click to start a new game
                ...
                if event.pos == (lastMouseDownX, lastMouseDownY):
                    ...
                    firstSelectedGem = checkForGemClick((lastMouseDownY, lastMouseDownY))
                    clickedSpace = checkForGemClick(event.pos)
        ...
        if clickedSpace and not firstSelectedGem:
            ...
            if firstSwappingGem == None and secondSwappingGem == None:
                #If both are None, then the gams were not adjacent
            ...
            if matchedGems == []:
                ...
                while matchedGems != []:
                    ...
                    #where on the screen to display text to show how many points the player got.
                    #points is a list because if the player gets multiple matches,
                    #then multiple points text should appear
                    points = []
        ...
        if gameIsOver:
            if clickContinueTextSurf == None:
                #Only render the text once. In future iterations, just use the Surface object already
                #in clickContinueTextSurf
...
def getSwappingGems(board, firstXY, secondXY):
    #If the games at th (X, Y) coordinates of the two gems are adjacent,
    #then their 'direction' keys are set to the appropriate direction value to be swapped with
    #each ohter. Otherwise, (None, None) is returned.
...
def getDropSlots(board):
    #Creates a "drop slot" for each column and fills the slot with a number of gems that that column
    #is lacking. This function assumes that the gems have been gravity dropped already
        ...
        for y in range(BOARDHEIGHT-1, -1, -1): #start from bottom, going up
            if boardCopy[x][y] == EMPTY_SPACE:
                ...
                for offsetX, offsetY in ((0, -1), (1, 0), (0, 1), (-1, 0)):
                    #Narrow down the possible gems we should put in the blank space so we don't
                    #end up putting any two of the same gems next to each other when they drop
...
def findMatchingGems(board):
    ...
    #loop through each space, checking for 3 adjacent identiacal gems
    for x in range(BOARDWIDTH):
        for y in range(BOARDHEIGHT):
            ...
            if getGemAt(boardCopy, x, y) == getGemAt(boardCopy, x+1, y) == \
                ...
                while getGemAt(boardCopy, x+offset, y) == targetGem:
                    #keep checking, in casae there's more than 3 gems in a row
...
def animateMovingGems(board, gems, pointsText, score):
    ...
    while progress < 100: #animation loop
        ...
        pygame.display.update()
        FPSCLOCK.tick(FPS)
        progress += MOVERATE #progress the animation a litle bit more for the next frame
...
def fillBoardAndAnimate(board, points, score):
    ...
    while dropSlots != [[]] * BOARDWIDTH:
        ...
        #the lowest by deleteing the previous lowest gems
        for x in range(len(dropSlots)):
            if len(dropSlots[x]) == 0:
                continue
...
def getBoardCopyMinusGems(board, gems):
    #Creates and returns a copy of the passed board data structure with the gems in the "gems" list
    #removed from it. Gems is a list of dicts, with keys x, y, direction, imageNum
