...
def main():
    ...
    while True: #main game loop
        ...
        if paletteClicked != None and paletteClicked != lastPaletteClicked:
            #a palette button was clicked that is different from the last palette button clicked
            #(this check prevents the player from accidentally clicking the same palette twice)
...
def showSettingsScreen():
    ...
    #The pixel coordinates in this function were obtained by loading the inkspillsettings.png image
    #into a graphics editor and reading the pixel coordinates from there. Handy trick.
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                for i in range(len(COLORSCHEMES)):
                    ...
                    if pygame.Rect(500, 30 + i * 60, MEDIUMBOXSIZE * 3, MEDIUMBOXSIZE * 2) \
...
def flashBorderAnimation(color, board, animationSpeed=30):
    ...
    for start, end, step in ((0, 256, 1), (255, 0, -1)):
        #the first iteration on the outer loop will set the inner loop to have transparency go
        #from 0 to 255, the second iteration will have to go from 255 to 0. This is the "flash"
...
def drawBoard(board, transparency=255):
    #The colored squares are drawm to a temporary surface which is then
...
def drawPalettes():
    #Draws the siz color palettes at the bottom of the screen
...
def getColorOfPaletteAt(x, y):
    #Returns the index of the color in paletteColrs that the x and y parameters are over
    ...
    for i in range(numColors):
        ...
        if r.collidepoint(x, y):
            return i
    return None #no palette exits at these x, y coordinates
...
def floodFill(board, oldColor, newColor, x, y):
    ...
    if x> 0:
        floodFill(board, oldColor, newColor, x-1, y) #on box to left
    if x < boardWidth-1:
        floodFill(board, oldColor, newColor, x+1, y) #on box to right
    if y> 0:
