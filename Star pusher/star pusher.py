...
#The percentage of outdoor tiles that have additional decoration on them, such as tree or rock
...
def main():
    global FPSCLOCK, DISPLAYSURF, IMAGESDICT, TILEMAPPING, OUTSIDEDECOMAPPING, BASICFONT, \
...
def isBlocked(mapObj, gameStateObj, x, y):
    #Returns True if the (x, y) position on the map is blocked by a wall or star,
    ...
    for lineNum in range(len(content)):
        ...
        if line != '':
            ...
            assert len(stars) >= len(goals), \
                'Level %s (around line %s) in %s is impossible to solve it. ' \
...
def drawMap(mapObj, gameStateObj, goals):
    ...
    for x in range(len(mapObj)):
        for y in range(len(mapObj[x])):
            spaceRect = pygame.Rect((x * TILEWIDTH, y * (TILEHEIGHT - TILEFLOORHEIGHT),
            ...
            #Last draw this player on the board
            if (x, y) == gameStateObj['player']:
                #Note: The value "currentImage" refers to a key in "PLAYERIMAGES"
                #which has the specific player image we want to show
